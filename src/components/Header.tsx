...
const Header = () => {
  ...
  const navigationItems = [
    ...
    { name: 'Dentistry', hasDropdown: true },
    { name: 'Invisalign®', hasDropdown: true },
              ...
              <ul className="flex space-x-8">
                {navigationItems.map((item) => (
                  <li key={item.name} className="relative group">
                    <button className={`font-sansation-bold hover:text-[#3e8bb2] active:text-[#3e8bb2] transition-all duration-300 flex items-center space-x-1 ${
                      isScrolled 
                    ...
                    {item.hasDropdown && (
                      <div className="absolute top-full left-0 mt-2 w-48 bg-white shadow-lg rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                        <div className="py-2">
          ...
          <div className="lg:hidden border-t border-gray-200 py-4">
            <nav>
              <ul className="space-y-2">
                {navigationItems.map((item) => (
                  <li key={item.name}>
                    <button className="text-[#004681] font-sansation-bold text-[18px] hover:text-[#3e8bb2] active:text-[#3e8bb2] transition-colors w-full text-left py-2 flex items-center justify-between">
                      <span>{item.name}</span>
                      {item.hasDropdown && (
                        <svg 
                          className="w-5 h-5" 
